...
// Mantém compatibilidade com seus dados reais: os coeficientes/curvas
// refletem relações calibradas no processo (temperatura, tempo, pressão, velocidade).
...
export type Params = {
  ...
  press: number;  // 95..110 (bar/atm conforme seu padrão)
  speed: number;  // 250..350 (rpm ou unidade do processo)
};

export type Prediction = {
  quality: number; // 300..400 (índice de qualidade)
  energy: number;  // 350..700 (kWh/ton, ajustável)
};
...
// RNG sementeado (xorshift32) para simulação reprodutível
...
// -------- Núcleo: predição base (calibrada) --------
function basePrediction(p: Params, noiseStd = 2, rng?: () => number): Prediction {
  ...
  // QUALIDADE — estrutura compatível com o seu modelo atual
  let quality =
    320 +
    45 * (0.3 + 0.7 * Math.pow(tN, 0.8)) +                         // temperatura (mais forte)
    25 * Math.exp(-Math.pow(tiN - 0.65, 2) / 0.3) +                // tempo (curva ótima)
    20 * (prN + 0.3 * Math.sin(prN * Math.PI * 2)) +               // pressão (limiares)
    15 * (Math.sqrt(spN) + 0.2 * Math.cos(spN * Math.PI)) +        // velocidade (mistura)
    8  * tN * tiN + 4 * prN * spN + 3 * tN * prN;                  // interações
  ...
  // ENERGIA — simples e coerente (ajuste se tiver curva medida)
  let energy =
    400 +
    120 * tN +
    40  * tiN +
...
// -------- Modelo de Produção (Inferência) --------
export class MetallurgyInferenceModel implements Model {
  predict(p: Params): Prediction {
    // ruído baixo ~ variabilidade de processo
    return basePrediction(p, 2);
  }
}

// -------- Modelo de Simulação (realista, não-otimista) --------
export class MetallurgySimulationModel implements Model {
  ...
  private minQ = 340, maxQ = 370;  // faixa típica da simulação
  ...
  predict(p: Params): Prediction {
    // ruído heterocedástico: mais variação fora do "doce"
...
// -------- (Opcional) Amostragem LHS para gerar cenários --------
