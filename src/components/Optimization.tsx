...
export const Optimization: React.FC<Props> = ({ t, isDark, onOptimizationComplete }) => {
  ...
  // >>> NOVO: progresso e cancelamento específicos da BO (worker)
  const [boProgress, setBoProgress] = React.useState<number | null>(null); // 0..budget
  ...
  function energyBadge(e: number) {
    ...
    return { label: 'Ineficiente', class: isDark ? 'bg-rose-900/50 text-rose-200 border-rose-700' : 'bg-rose-100 text-rose-700 border-rose-200' };
  }
  ...
  // Cancelamento opcional (se quiser expor um botão no futuro)
  const cancelBO = React.useCallback(() => {
    if (boWorkerRef.current) {
      boWorkerRef.current.postMessage({ type: 'cancel' });
      try { boWorkerRef.current.terminate(); } catch {}
      boWorkerRef.current = null;
      setRunningBO(false);
      setBoProgress(null);
  ...
  async function executar(method: OptimizeMethod) {
    ...
    // >>> Métodos GRID e GA (comportamento original)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:col-span-3">
          ...
          <div className={`rounded-2xl border bg-gradient-to-br ${gradBlue} p-5 transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5 ${ringBlue}`}>
            ...
            {/* >>> NOVO: barra de progresso da BO (worker) */}
            {runningBO && (
              <div className="mt-3">
                <div className="flex items-center justify-between text-xs text-gray-500 mb-1">
                  <span>Progresso</span>
                  <span>{boProgress !== null ? Math.min(100, Math.round(((boProgress ?? 0) / Math.max(1, budget)) * 100)) : 0}%</span>
                </div>
                <div className={`h-2 w-full rounded-full ${isDark ? 'bg-gray-700' : 'bg-gray-200'}`}>
                  <div
                    className="h-2 rounded-full bg-blue-500 transition-all"
                    style={{ width: `${boProgress !== null ? Math.min(100, ((boProgress ?? 0) / Math.max(1, budget)) * 100) : 0}%` }}
                  />
                </div>
                ...
                <div className="text-right mt-2">
                  <button
                    onClick={cancelBO}
                    className="text-xs px-2 py-1 rounded-md border border-blue-300 text-blue-700 hover:bg-blue-50 disabled:opacity-50"
                  >
                    Cancelar
                  </button>
                </div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full appearance-none bg-transparent
                  ...
                  [&::-webkit-slider-thumb]:shadow-[0_0_10px_rgba(16,185,129,0.6)]
                  [&::-webkit-slider-runnable-track]:h-1.5 [&::-webkit-slider-runnable-track]:rounded-full
                  [&::-webkit-slider-runnable-track]:bg-gray-300 dark:[&::-webkit-slider-runnable-track]:bg-gray-700"
              />
            </div>
