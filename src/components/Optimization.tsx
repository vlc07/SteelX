...
export const Optimization: React.FC<Props> = ({ t, isDark, onOptimizationComplete }) => {
  ...
  // Progresso visual da BO (main thread livre)
  ...
  // ---- Worker: progress fake incremental (suave) enquanto a BO roda ----
  function startBOProgress() {
    setBoProgress(0);
    if (boTickerRef.current) window.clearInterval(boTickerRef.current);
    // incrementa em passos suaves até ~92% para evitar travar em 0%
    const id = window.setInterval(() => {
      setBoProgress(prev => {
        const limit = 92;
        if (prev >= limit) return prev;
        const step = Math.random() * 2 + 0.5; // 0.5% a 2.5%
        return Math.min(limit, prev + step);
      });
    }, 140);
  ...
  async function executar(method: OptimizeMethod) {
    ...
    if (method === 'bo') {
      // Executa em Worker + progresso visual
      setRunningBO(true);
      startBOProgress();
      try {
    ...
    // GRID/GA (main thread como antes)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:col-span-3">
          ...
          <div className={`rounded-2xl border bg-gradient-to-br ${gradBlue} p-5 transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5 ${ringBlue}`}>
            ...
            {/* Barra de progresso da BO (não altera layout/tema existente) */}
            {runningBO && (
              <div className="mt-4">
          ...
          <div className={`flex items-center justify-between px-6 py-5 ${isDark ? 'bg-gray-900/40' : 'bg-green-100/80'}`}>
            ...
            <div className="text-right">
              <div className="flex items-center justify-end gap-2">
                ...
                <span
                  className={`cursor-help text-xs ${isDark ? 'bg-blue-900/60 text-blue-200 border border-blue-800' : 'bg-blue-100 text-blue-700 border-blue-200'} px-2 py-0.5 rounded-full`}
