...
export const Optimization: React.FC<Props> = ({ t, isDark, onOptimizationComplete }) => {
  ...
  // Base de cartão + GLOW premium
  const cardBase = `${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl border p-6 md:p-7 transition-all duration-200`;
  const glowBlue = isDark
    ? 'hover:ring-2 hover:ring-blue-400/30 hover:shadow-[0_10px_30px_-12px_rgba(59,130,246,.35)] hover:-translate-y-0.5'
    : 'hover:ring-2 hover:ring-blue-300/35 hover:shadow-[0_10px_30px_-14px_rgba(30,64,175,.25)] hover:-translate-y-0.5';
  const glowEmerald = isDark
    ? 'hover:ring-2 hover:ring-emerald-400/30 hover:shadow-[0_10px_30px_-12px_rgba(16,185,129,.35)] hover:-translate-y-0.5'
    : 'hover:ring-2 hover:ring-emerald-300/35 hover:shadow-[0_10px_30px_-14px_rgba(16,185,129,.25)] hover:-translate-y-0.5';
  const glowSlate = isDark
    ? 'hover:ring-2 hover:ring-slate-400/25 hover:shadow-[0_10px_30px_-12px_rgba(148,163,184,.25)] hover:-translate-y-0.5'
    : 'hover:ring-2 hover:ring-slate-300/30 hover:shadow-[0_10px_30px_-14px_rgba(148,163,184,.22)] hover:-translate-y-0.5';
  ...
  function badgeFor(name: keyof typeof bounds, v: number) {
    ...
    if (v > id.high) return { label: 'Alto',  class: isDark ? 'bg-rose-900/50 text-rose-200 border border-rose-700'   : 'bg-rose-100 text-rose-700 border border-rose-200' };
  ...
  return (
    <div className="space-y-6">
      {/* Cabeçalho (com glow azul) */}
      <div className={`${cardBase} ${glowBlue}`}>
      ...
      {/* Presets */}
      <div className={`${cardBase} ${glowSlate} pt-5`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            {key:'resistencia', icon:<Flame className="h-5 w-5 text-rose-500"/>, title:'Alta Resistência',
             desc:'Foco em qualidade. T e tempo mais altos (λ baixo).', chip:'λ ≈ 0.08 · Qualidade ≥ 365', chipClass:'bg-rose-100 text-rose-700 border border-rose-200'},
            {key:'ductilidade', icon:<Dna className="h-5 w-5 text-green-500"/>, title:'Alta Ductilidade',
             desc:'Maleabilidade com boa qualidade. T/tempo moderados.', chip:'λ ≈ 0.12 · Qualidade ≥ 360', chipClass:'bg-green-100 text-green-700 border border-green-200'},
            {key:'energia', icon:<Leaf className="h-5 w-5 text-emerald-500"/>, title:'Economia de Energia',
             desc:'Reduz custo/CO₂. T/tempo menores (λ mais alto).', chip:'λ ≈ 0.22 · Qualidade ≥ 355', chipClass:'bg-emerald-100 text-emerald-700 border border-emerald-200'},
            {key:'balanceado', icon:<Zap className="h-5 w-5 text-blue-500"/>, title:'Balanceado',
             desc:'Equilíbrio padrão. Você ajusta depois, se quiser.', chip:'λ ≈ 0.15 · Qualidade mínima opcional', chipClass:'bg-blue-100 text-blue-700 border border-blue-200'},
          ].map(p => (
            <button
              key={p.key}
              onClick={() => applyPreset(p.key as any)}
              className={`${cardBase} ${glowBlue} text-left`}
            >
              <div className="flex items-center gap-2">
                {p.icon}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:col-span-3">
          ...
          <div className={`${cardBase} ${glowBlue}`}>
            ...
            <p className={`${sub} text-sm mb-4`}>Testa várias combinações de parâmetros como se fosse uma tabela.</p>
            <button
          ...
          <div className={`${cardBase} ${glowBlue}`}>
            ...
            <p className={`${sub} text-sm mb-4`}>Mistura e seleciona os melhores parâmetros a cada rodada.</p>
            <button
              onClick={() => executar('ga')}
              disabled={runningGA}
              className={`w-full py-3 rounded-lg font-semibold flex items-center justify-center gap-2 transition ${
          ...
          <div className={`${cardBase} ${glowBlue}`}>
            ...
            <p className={`${sub} text-sm mb-4`}>Aprende com cada teste e sugere próximos parâmetros.</p>
            <button
              onClick={() => executar('bo')}
              disabled={runningBO}
              className={`w-full py-3 rounded-lg font-semibold flex items-center justify-center gap-2 transition ${
        ...
        {/* Configurações (glow esmeralda) */}
        <div className={`${cardBase} ${glowEmerald}`}>
          ...
          <div className="mb-5">
            <label className={`block text-sm mb-1 ${sub}`}>Budget (nº de testes)</label>
          ...
          <div className="mb-5">
            <label className={`block text-sm mb-1 ${sub}`}>
          ...
          <div className={`${useQualityConstraint ? '' : 'opacity-50 pointer-events-none'}`}>
            <label className={`block text-sm mb-1 ${sub}`}>Qualidade mínima</label>
      ...
      {/* Faixas de otimização (glow neutro) */}
      <div className={`${cardBase} ${glowSlate}`}>
        <div className="flex items-center justify-between mb-3">
          <h3 className={`font-semibold ${text}`}>Faixas de Otimização</h3>
      ...
      {last && (
        <div className={`rounded-2xl border overflow-hidden ${isDark ? 'border-green-700 bg-gradient-to-br from-gray-800 to-gray-900' : 'border-green-200 bg-gradient-to-br from-green-50 to-white'}`}>
          <div className={`flex items-center justify-between px-6 py-5 ${isDark ? 'bg-gray-900/40' : 'bg-green-100/80'}`}>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-xs ${sub}`}>{last.evaluations} testes realizados</p>
              </div>
            </div>
            <div className="text-right">
              <div className="flex items-center justify-end gap-2">
                <span className={`text-xs ${sub}`}>Score</span>
                <span className={`cursor-help text-xs ${isDark ? 'bg-blue-900/60 text-blue-200 border border-blue-800' : 'bg-blue-100 text-blue-700 border border-blue-200'} px-2 py-0.5 rounded-full`} title="Score combina qualidade e energia conforme λ.">ℹ️</span>
              </div>
          ...
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-xl p-4 shadow-sm`}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`px-2.5 py-1 rounded-full text-[11px] font-semibold ${energyBadge(last.energy).class}`}><BatteryCharging className="inline h-3 w-3 mr-1"/>{energyBadge(last.energy).label}</span>
                </div>
            ...
            <div className={`text-sm ${sub}`}>💡 Ajuste o controle “Equilíbrio entre qualidade e energia” para priorizar custo/CO₂ ou qualidade.</div>
          </div>
        </div>
      )}

      {/* Histórico (glow neutro + títulos visíveis) */}
      <div className={`${cardBase} ${glowSlate}`}>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
        ...
        {history.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {history.map(item => (
              <div
                key={item.id}
                className={`${cardBase} ${glowSlate} !p-5 border ${isDark ? 'bg-gray-900/40 border-gray-700' : 'bg-gray-50 border-gray-200'}`}
              >
...
function ParamCard(props: {
  ...
  return (
    <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-xl p-4 shadow-sm transition-all ${isDark ? 'hover:shadow-[0_12px_30px_-12px_rgba(16,185,129,.28)]' : 'hover:shadow-[0_12px_30px_-14px_rgba(16,185,129,.22)]'} hover:-translate-y-0.5`}>
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
...
function RangeCard({
  ...
  return (
    <div className={`${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl p-4 border shadow-sm transition-all ${isDark ? 'hover:shadow-[0_12px_30px_-12px_rgba(148,163,184,.25)]' : 'hover:shadow-[0_12px_30px_-14px_rgba(148,163,184,.20)]'} hover:-translate-y-0.5`}>
      <div className="flex items-center justify-between">
